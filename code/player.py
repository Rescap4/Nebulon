...
class Player(AnimatedSprite):
    def __init__(self, pos, frames, groups, collision_sprites, battery_sprites, tablet_sprites, slide_objects, serial_num):
        ...
        self.is_next = False
        self.can_move = False
        self.slowed_flag = False

        self.old_pos = (self.rect.x, self.rect.y)
        self.old_rect = self.rect.copy()

        # Timers
        self.move_delay = Timer(150)
        self.push_delay = Timer(300)
        self.impact_face = Timer(400)
        self.dead_face = Timer(800)
    ...
    def wall_collision(self, direction):
        ...
        if self.slowed_flag:
            ...
            # Check variation
            if abs(current_pos[0] - self.old_pos[0]) >= 64 or abs(current_pos[1] - self.old_pos[1]) >= 64 or self.direction == [0,0]:
                self.slowed_flag = False
        else:
            self.speed = 1600
