...
TEXT_HINT = {'fr':{
    ...
    8: """Niveau 8  Une boîte aide à accéder à des endroits auparavant inaccessibles. Il faut parfois en pousser une puis aller dans une autre direction""",
    9: """Niveau 9  Une boîte est arrêtée par une batterie, cela peut être utilisé pour gagner accès d'autres zones du niveaux""",
    10: """Niveau 10  La position des piques permettent parfois de déduire la sortie""",
    ...
    13: """Niveau 13  Une boîte permet de protéger des piques""",
    14: """Niveau 14  Il faut détruire tout les blocs du centre pour sortir""",
    ...
    18: """Niveau 18  Deux nebulons doivent collaborer jusqu'à ce qu'ils puissent sortir ensemble""", #Il est inutile de se rendre dans la crevasse en haut à gauche, mais il est requis de se mettre à son entrée
    19: """Niveau 19  Les blocs du centre font parti de la solution""",
    20: """Niveau 20  La batterie de gauche est la troisième à collecter""",
    ...
    23: """Niveau 23  La partie supérieure du niveau est un piège, la partie du bas est la seule qui peut aider à sortir les deux nebulons""",
    ...
    31: """Niveau 31  Pour collecter les batteries d'un côté à l'autre, il faut avancer le tunnel du milieu une case à la fois. Pour progresser, il faut se retenir de collecter les batteries dès que possible et placer d'abord l'autre nebulon""",
    ...
    6: """Level 6  The order in which batteries are collected always matter""",
    7: """Level 7  Some areas become inaccessible from the very first move""",
    8: """Level 8  A box is useful to reach previously inaccessible areas. It is also useful to push a box and go in another direction""",
    9: """Level 9  A box is stopped by a battery; this can be used to gain access to other areas of the level""",
    10: """Level 10  The position of spikes sometimes helps deduce the exit""",
    ...
    13: """Level 13  A box can be protection from spikes""",
    14: """Level 14  All blocks in the center needs to be destroyed""",
    ...
    18: """Level 18  Two nebulons must collaborate until they can both exit together""",
    19: """Level 19  The blocks in the middle are part of the solution""",
    20: """Level 20  The left battery needs to be collected third""",
    ...
    23: """Level 23  The upper side is a trap, focus on the bottom of the level to align both nebulons on the exit row""",
    ...
    28: """Level 28  Getting out requires four consecutive collisions with the three boxes""",
    29: """Level 29  Deduce wich battery needs to be collected first but more importantly wich battery needs to be collected last""",
    30: """Level 30  Collect the battery from its left side, then destroy the blocks, followed by the top of the column and end with the rest of the column""",
    31: """Level 31  To collect the batteries from one side to the other, advance the middle tunnel one tile at a time. To progress, you need to hold back from collecting batteries right away and move the other nebulon to the right place first""",
    32: """Level 32  Using a nebulon to push the other is done at least four times, it is essential to save both""",
